<x-layouts.admin>
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-[#0f2640]">Editar Projeto</h1>
            <p class="text-slate-400 text-sm">{{ $projeto->titulo }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.projetos.update', $projeto) }}" class="space-y-6 max-w-3xl">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="titulo" value="{{ old('titulo', $projeto->titulo) }}" required maxlength="150"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="tipo" required
                        ...
                        <option value="consultoria" {{ old('tipo', $projeto->tipo) === 'consultoria' ? 'selected' : '' }}>Consultoria</option>
                        <option value="formacao"    {{ old('tipo', $projeto->tipo) === 'formacao'    ? 'selected' : '' }}>Formação</option>
                        <option value="equipamentos"{{ old('tipo', $projeto->tipo) === 'equipamentos'? 'selected' : '' }}>Equipamentos</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Local</label>
                    <input type="text" name="local" value="{{ old('local', $projeto->local) }}" maxlength="100"
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#1a3a5c]/20 focus:border-[#1a3a5c] outline-none transition">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="descricao" rows="4" required
                          class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#1a3a5c]/20 focus:border-[#1a3a5c] outline-none transition resize-none">{{ old('descricao', $projeto->descricao) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1.5">Resultado / Métrica</label>
                <input type="text" name="resultado" value="{{ old('resultado', $projeto->resultado) }}" maxlength="250"
                       class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#1a3a5c]/20 focus:border-[#1a3a5c] outline-none transition">
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="mb-2 rounded-lg overflow-hidden h-20 bg-slate-100">
                        <img src="/img/{{ $projeto->foto }}" class="h-full w-full object-cover">
                    </div>
                    @endif
                    <input type="text" name="foto" value="{{ old('foto', $projeto->foto) }}" maxlength="150"
                           placeholder="ex: 4.jpeg"
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#1a3a5c]/20 focus:border-[#1a3a5c] outline-none transition">
                </div>
                <div x-data="{ cor: '{{ old('cor', $projeto->cor) }}' }">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="ordem" value="{{ old('ordem', $projeto->ordem) }}" min="0"
