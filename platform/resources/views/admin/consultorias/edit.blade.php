    ...
    <form method="POST" action="{{ route('admin.consultorias.update', $consultoria) }}" class="space-y-6 max-w-3xl">
        ...
        <div class="bg-indigo-50 rounded-2xl border border-indigo-200 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="titulo_fr" value="{{ old('titulo_fr', $consultoria->getRawOriginal('titulo_fr')) }}"
                ...
                <div>
                    ...
                    <input type="text" name="tagline_fr" value="{{ old('tagline_fr', $consultoria->getRawOriginal('tagline_fr')) }}"
        ...
        <div class="bg-indigo-50 rounded-2xl border border-indigo-200 p-6 space-y-4">
            ...
            <textarea name="features_raw_fr" rows="7"
                      class="w-full border border-indigo-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400/30 focus:border-indigo-400 font-mono resize-y">{{ old('features_raw_fr', implode("\n", $consultoria->getRawOriginal('features_fr') ? (json_decode($consultoria->getRawOriginal('features_fr'), true) ?? []) : [])) }}</textarea>
        </div>
