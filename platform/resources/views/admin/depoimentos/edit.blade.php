<x-layouts.admin>
    <x-slot name="title">Editar Depoimento</x-slot>
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-[#0f2640]">Editar Depoimento</h1>
            <p class="text-slate-400 text-sm">{{ $depoimento->nome }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.depoimentos.update', $depoimento) }}" class="space-y-6 max-w-2xl">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nome" value="{{ old('nome', $depoimento->nome) }}" required maxlength="100"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="cargo" value="{{ old('cargo', $depoimento->cargo) }}" required maxlength="100"
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#1a3a5c]/20 focus:border-[#1a3a5c] outline-none transition">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Empresa *</label>
                    <input type="text" name="empresa" value="{{ old('empresa', $depoimento->empresa) }}" required maxlength="100"
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#1a3a5c]/20 focus:border-[#1a3a5c] outline-none transition">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="texto" rows="5" required
                          class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#1a3a5c]/20 focus:border-[#1a3a5c] outline-none transition resize-none">{{ old('texto', $depoimento->texto) }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Classificação (1–5)</label>
                    <input type="number" name="rating" value="{{ old('rating', $depoimento->rating) }}" min="1" max="5"
