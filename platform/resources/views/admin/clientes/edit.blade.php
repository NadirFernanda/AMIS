<x-layouts.admin>
    <x-slot name="title">Editar — {{ $cliente->name }}</x-slot>

    <div class="mb-6">
        <a href="{{ route('admin.clientes.show', $cliente) }}" class="text-sm text-slate-500 hover:text-[#1a3a5c] flex items-center gap-1.5 transition-colors">
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl border border-slate-200 p-8">
            <h2 class="text-lg font-bold text-[#0f2640] mb-6">Editar Cliente</h2>

            <form method="POST" action="{{ route('admin.clientes.update', $cliente) }}" class="space-y-5">
                @csrf @method('PUT')

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-1.5">Nome Completo *</label>
                        <input type="text" name="name" required value="{{ old('name', $cliente->name) }}"
                    ...
                    <div>
                        ...
                        <input type="email" name="email" required value="{{ old('email', $cliente->email) }}"
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        @foreach(['pendente', 'ativo', 'inativo'] as $s)
                        <option value="{{ $s }}" {{ old('status', $cliente->status) === $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                        @endforeach
                    </select>
                </div>

                <hr class="border-slate-100">
                <p class="text-xs text-slate-400">Deixe em branco para manter a palavra-passe atual.</p>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5" x-data="{ show: false }">
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-1.5">Nova Palavra-passe</label>
                        <div class="relative">
                            <input :type="show ? 'text' : 'password'" name="password"
                                class="w-full px-4 py-3 pr-11 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3a5c]/30 focus:border-[#1a3a5c]"
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-1.5">Confirmar Nova Palavra-passe</label>
