<x-layouts.admin>
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-[#0f2640]">Editar: {{ $fornecedor->nome_empresa }}</h1>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.fornecedores.update', $fornecedor) }}" class="space-y-6 max-w-3xl">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="nome_empresa" value="{{ old('nome_empresa', $fornecedor->nome_empresa) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="pais" value="{{ old('pais', $fornecedor->pais) }}" required
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0f2640]/20 focus:border-[#0f2640]">
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="url" name="website" value="{{ old('website', $fornecedor->website) }}" placeholder="https://"
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0f2640]/20 focus:border-[#0f2640]">
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <textarea name="descricao" rows="4" required
                          class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0f2640]/20 focus:border-[#0f2640] resize-none">{{ old('descricao', $fornecedor->getRawOriginal('descricao')) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
            ...
            @else
            @php $checked = old('equipamentos', $selecionadas); @endphp
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <label class="flex items-center gap-2 cursor-pointer select-none border border-slate-200 rounded-xl px-3 py-2 hover:bg-slate-50">
                    <input type="checkbox" name="equipamentos[]" value="{{ $cat->id }}"
                           {{ in_array($cat->id, $checked) ? 'checked' : '' }}
        ...
        <div class="bg-blue-50 rounded-2xl border border-blue-200 p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="descricao_en" rows="3"
                          class="w-full border border-blue-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 resize-none">{{ old('descricao_en', $fornecedor->getRawOriginal('descricao_en')) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-indigo-50 rounded-2xl border border-indigo-200 p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="descricao_fr" rows="3"
                          class="w-full border border-indigo-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400/30 focus:border-indigo-400 resize-none">{{ old('descricao_fr', $fornecedor->getRawOriginal('descricao_fr')) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-5">
                <div x-data="{ cor: '{{ old('cor', $fornecedor->cor) }}' }">
                ...
                <div>
                    ...
                    <input type="number" name="ordem" value="{{ old('ordem', $fornecedor->ordem) }}" min="0"
                           class="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0f2640]/20 focus:border-[#0f2640]">
                </div>
                <div class="pt-1">
                ...
                <div class="pt-1">
                    <label class="flex items-center gap-2 cursor-pointer select-none mt-6">
                        ...
                        <input type="checkbox" name="destaque" value="1"
                               {{ old('destaque', $fornecedor->destaque) ? 'checked' : '' }}
                               class="w-4 h-4 rounded border-slate-300 text-[#0f2640]">
