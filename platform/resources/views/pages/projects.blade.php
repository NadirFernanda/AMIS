<x-layouts.public>
    ...
    <x-slot name="description">{{ __('projects.page_desc') }}</x-slot>
    ...
    <div class="relative bg-[#1a3a5c] py-28 overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <span class="text-[#c9922a] text-sm font-semibold uppercase tracking-wider">{{ __('projects.header_label') }}</span>
            <h1 class="text-4xl sm:text-5xl font-extrabold text-white mt-2">{{ __('projects.header_title') }}</h1>
            <p class="text-slate-300 mt-4 max-w-2xl">{{ __('projects.header_desc') }}</p>
            {{-- Stats bar --}}
            <div class="flex flex-wrap gap-8 mt-10">
                @foreach([$projectos->count() . ' ' . __('projects.header_label'), __('projects.stat_countries'), __('projects.stat_trained'), __('projects.stat_satisfaction')] as $stat)
    ...
    <section class="py-24 bg-slate-50" x-data="{ filtro: 'todos' }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap gap-3 justify-center mb-14">
                @foreach(['todos' => __('projects.filter_all'), 'consultoria' => __('projects.filter_consulting'), 'formacao' => __('projects.filter_training'), 'equipamentos' => __('projects.filter_equipment')] as $val => $label)
    ...
    <section class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <span class="text-[#c9922a] text-sm font-semibold uppercase tracking-wider">{{ __('projects.testimonials_label') }}</span>
                <h2 class="text-3xl sm:text-4xl font-extrabold text-[#1a3a5c] mt-2">{{ __('projects.testimonials_title') }}</h2>
                <p class="text-slate-500 mt-4 max-w-xl mx-auto">{{ __('projects.testimonials_desc') }}</p>
            </div>
    ...
    <section class="py-16 bg-[#1a3a5c]">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 class="text-2xl sm:text-3xl font-extrabold text-white mb-4">{{ __('projects.cta_title') }}</h2>
            <p class="text-slate-300 mb-8 max-w-xl mx-auto">{{ __('projects.cta_desc') }}</p>
